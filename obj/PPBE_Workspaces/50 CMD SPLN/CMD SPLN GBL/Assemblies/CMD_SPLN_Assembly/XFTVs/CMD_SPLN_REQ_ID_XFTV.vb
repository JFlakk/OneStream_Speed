...
#Region "Get cWork OOC"
        Private Function Get_REQ_cWork_OOC() As TableView
			...
			xftv.EmptyRowsBackgroundColor = XFColors.AliceBlue
			Dim wfInfoDetails = Workspace.GBL.GBL_Assembly.GBL_Helpers.GetWFInfoDetails(si)
            Dim sql As String = $"SELECT CMD_SPLN_REQ_ID, 
										 WFScenario_Name, 
...
#Region "Get Create New REQ"
        Private Function Get_Create_New_Req() As TableView
			...
			xftv.EmptyRowsBackgroundColor = XFColors.AliceBlue
			Dim wfInfoDetails = Workspace.GBL.GBL_Assembly.GBL_Helpers.GetWFInfoDetails(si)
			Dim startYear = wfInfoDetails("TimeName")
            Dim sql As String = $"SELECT Req.Title,
							          Dtl.FY_1,
							          Dtl.FY_2,
							          Dtl.FY_3,
							          Dtl.FY_4,
            ...
            Using dbConnApp As DbConnInfoApp = BRApi.Database.CreateApplicationDbConnInfo(si)
                dt = BRApi.Database.ExecuteSql(dbConnApp,sql,False)
            End Using
'BRApi.ErrorLog.LogMessage(si,$"Hit Get Function Create")
			xftv.PopulateFromDataTable(dt,True,True)
    ...
    Using dbConnApp As DbConnInfoApp = BRApi.Database.CreateApplicationDbConnInfo(si)
        Using sqlConn As New SqlConnection(dbConnApp.ConnectionString)
            ...
            ' ************************************
            			...
            			For Each xftvRow As TableViewRow In xftv.Rows.Where(Function(r) Not r.IsHeader)
					Try
                    Dim req_ID_Val As Guid = xftvRow.Item("CMD_SPLN_REQ_ID").Value.xfconverttoGUID
                    Dim isInsert As Boolean = False

                    ' --- Find the DataRows ---
                    Dim targetRow As DataRow = dt.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}'").FirstOrDefault()
                        ...
                        Dim targetRowQuantity As DataRow = dt_Details.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}' AND Account = 'Quantity'").FirstOrDefault()
								If targetRowQuantity Is Nothing Then
									
  								  isInsert = True
							targetRowQuantity = dt_Details.NewRow()
				  			targetRowQuantity("CMD_SPLN_REQ_ID") = req_ID_Val
							...
							targetRowQuantity("Entity") = args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_SPLN_FundsCenter","")
							targetRowQuantity("Unit_of_Measure") = "Manpower"
            ...
            ' ************************************
            	For Each xftvRow As TableViewRow In xftv.Rows.Where(Function(r) Not r.IsHeader)
                    ...
                    Dim targetRow As DataRow = dt.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}'").FirstOrDefault()
						...
						Dim targetRowManpowerCost As DataRow = dt_Details.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}' AND Account = 'Manpower_Cost'").FirstOrDefault()
						...
						Dim targetRowManpower As DataRow = dt_Details.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}' AND Account = 'Manpower'").FirstOrDefault()
...
#Region"Save Create New Req"
   Public Function Save_Create_New_Req() As Boolean
	  ' Brapi.ErrorLog.LogMessage(si,"In Save Create")
         Dim xftv As New TableView
			xftv = args.TableView
			'Brapi.ErrorLog.LogMessage(si,"In Save Create 2")
...
'Brapi.ErrorLog.LogMessage(si,"In Save Create SQL 2")
            ...
            ' ************************************
						...
						Dim sEntity As String =  args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_SPLN_FundsCenter","")
						Dim entityLevel As String = Workspace.GBL.GBL_Assembly.GBL_Helpers.GetEntityLevel(si,sEntity)
						Dim sREQWFStatus As String = entityLevel & "_Formulate_PGM"
						Dim NewReqID As Guid = Guid.NewGuid()
						'BRapi.ErrorLog.LogMessage(si,"Hit")
            			For Each xftvRow As TableViewRow In xftv.Rows.Where(Function(r) Not r.IsHeader)
							For Each xftvCol As KeyValuePair(Of String, tableviewcolumn) In xftvRow.Items								
								'BRApi.ErrorLog.LogMessage(si,$"Hit: {xftvCol.Key} - {xftvCol.Value.Value}")
							Next
							
							Dim sU1APPNInput As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateAPPN","")
							Dim sU2Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateMDEP","")
							Dim sU3Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateAPEPT","")
							Dim sU4Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateDollarType","")
							Dim sU6Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateObjectClass","")
							...
							targetRowDetails("Entity") = args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_SPLN_FundsCenter","")
							...
							'BRApi.ErrorLog.LogMessage(si,"Hit")
							targetRowDetails("UD1") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateAPPN","")
							targetRowDetails("UD2") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateMDEP","")
							targetRowDetails("UD3") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateAPEPT","")
							targetRowDetails("UD4") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateDollarType","")
							targetRowDetails("UD5") = "None"
							targetRowDetails("UD6") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateObjectClass","")
...
'BRApi.ErrorLog.LogMessage(si,"Hit3")
							...
							targetRowDetails("Update_User") = si.UserName
								...
								targetRow("WFCMD_Name") = wfInfoDetails("CMDName")
								targetRow("WFTime_Name") = wfInfoDetails("TimeName")
								targetRow("Entity") = args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_SPLN_FundsCenter","")
								targetRow("REQ_ID") = Workspace.GBL.GBL_Assembly.GBL_REQ_ID_Helpers.Get_FC_REQ_ID(si,sEntity)
								targetRow("REQ_ID_Type") = "Requirement"
								targetRow("Title") = xftvRow.Item("Title").Value
								targetRow("APPN") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateAPPN","")
								targetRow("MDEP") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateMDEP","")
								targetRow("APE9") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateAPEPT","")
								targetRow("Dollar_Type") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateDollarType","")
								targetRow("Obj_Class") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_SPLN_FormulateObjectClass","")
            ...
            ' ************************************
				...
				Try
                    ...
                    Dim targetRow As DataRow = dt.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}'").FirstOrDefault()
						...
						Dim targetRowFunding As DataRow = dt_Details.Select($"CMD_SPLN_REQ_ID = '{req_ID_Val}' AND Account = 'REQ_Requested_Amt'").FirstOrDefault()

...
Public Class SQA_XFC_SubCMD_SPLN_REQ_Details
    ...
    Public Sub Fill_XFC_SubCMD_SPLN_REQ_Details_DT(adapter As SqlDataAdapter, dt As DataTable, selectQuery As String, ParamArray sqlparams() As SqlParameter)
    ...
    Public Sub Update_XFC_SubCMD_SPLN_REQ_Details(dt As DataTable, adapter As SqlDataAdapter)
        Using transaction As SqlTransaction = _connection.BeginTransaction()
            ' Insert command
            Dim insertQuery As String = """
                INSERT INTO XFC_SubCMD_SPLN_REQ_Details (
                    SubCMD_SPLN_REQ_ID, WFScenario_Name, WFCMD_Name, WFTime_Name, Unit_of_Measure, Entity, IC, Account, Flow,
            ...
            Dim updateQuery As String = """
                UPDATE XFC_SubCMD_SPLN_REQ_Details SET
                ...
                WHERE SubCMD_SPLN_REQ_ID = @SubCMD_SPLN_REQ_ID AND Fiscal_Year = @Fiscal_Year AND Unit_of_Measure = @Unit_of_Measure AND Account = @Account;"""
            adapter.UpdateCommand = New SqlCommand(updateQuery, _connection, transaction)
            AddParameters(adapter.UpdateCommand, True)

            ' Delete command
            Dim deleteQuery As String = "DELETE FROM XFC_SubCMD_SPLN_REQ_Details WHERE SubCMD_SPLN_REQ_ID = @SubCMD_SPLN_REQ_ID AND Fiscal_Year = @Fiscal_Year AND Unit_of_Measure = @Unit_of_Measure AND Account = @Account;"

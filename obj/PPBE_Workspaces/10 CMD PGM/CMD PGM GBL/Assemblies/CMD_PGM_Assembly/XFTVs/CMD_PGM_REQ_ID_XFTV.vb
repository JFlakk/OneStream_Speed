...
Namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.Spreadsheet.CMD_PGM_REQ_ID_XFTV
	...
	'BRApi.ErrorLog.LogMessage(si,$"Hit {args.CustSubstVarsAlreadyResolved.count()}")
						...
						Else If args.TableViewName = "MM_PEG"
							Return Get_REQ_MM_PEG()
							
						Else If args.TableViewName = "Gen_Cmt"
							'SubstVars_List.Add("BL_CMD_PGM_REQTitleList")
                      		Return Get_Gen_Cmt()
						End If
...
#End Region
#Region "Get MM PEG"
        Private Function Get_REQ_MM_PEG() As TableView
            Dim dt As New DataTable()
            Dim xftv As New TableView()
			Dim ReqID As String = args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_PGM_REQTitleList")
				If String.IsNullOrWhiteSpace(ReqID)
            ...
            Dim sql As String = $"SELECT Req.CMD_PGM_REQ_ID, 
										 Req.WFScenario_Name, 
										 Req.WFCMD_Name, Req.WFTime_Name, Req.REQ_ID, Req.Group_Name,Req.Cat1, Req.Cat2,Req.Cat3,Req.Cat4,Req.RQMT_Driver,Req.Cost_Factor,
										Req.Factor_Type,Req.Field_Description, Req.MMPEG_Issue,Req.Post_COEX_Issue,Req.Inflation_Type
		                		FROM XFC_CMD_PGM_REQ Req
							    WHERE Req.WFScenario_Name = '{wfInfoDetails("ScenarioName")}'
								AND Req.WFCMD_Name = '{wfInfoDetails("CMDName")}'
								AND Req.WFTime_Name = '{wfInfoDetails("TimeName")}'
...
'Brapi.ErrorLog.LogMessage(si,"SQL" & sql )
        ...
        End Function
#End Region
#Region "Get General Comment"
        Private Function Get_Gen_Cmt() As TableView
            Dim dt As New DataTable()
            Dim xftv As New TableView()
			Dim ReqID As String = args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_PGM_REQTitleList")
				If String.IsNullOrWhiteSpace(ReqID)
...
'Brapi.ErrorLog.LogMessage(si, "ID" & RequirementID)
            ...
            Dim sql As String = $"SELECT Req.CMD_PGM_REQ_ID,Req.WFScenario_Name, 
										 Req.WFCMD_Name, Req.WFTime_Name,
										 Req.REQ_ID,Req.Gen_Comment_Tab,
...
'BRApi.ErrorLog.LogMessage(si,$"Hit Save 3")
						Dim sqlparams As SqlParameter() = New SqlParameter() {
							...
							New SqlParameter("@REQ_ID", SqlDbType.NVarChar) With {.Value = REQ_ID_List(1)}
						}
...
'Brapi.ErrorLog.LogMessage(si,"In Save Create SQL 2")
            ...
            ' ************************************
            			...
            			For Each xftvRow As TableViewRow In xftv.Rows.Where(Function(r) Not r.IsHeader)
							...
							Dim sU1APPNInput As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateAPPN","")
							Dim sU2Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateMDEP","")
							Dim sU3Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateAPEPT","")
							Dim sU4Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateDollarType","")
							Dim sU6Input As String = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateObjectClass","")
							...
							'BRApi.ErrorLog.LogMessage(si,"Hit")
							targetRowDetails("UD1") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateAPPN","")
							targetRowDetails("UD2") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateMDEP","")
							targetRowDetails("UD3") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateAPEPT","")
							targetRowDetails("UD4") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateDollarType","")
							targetRowDetails("UD5") = "None"
							targetRowDetails("UD6") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateObjectClass","")
...
'BRApi.ErrorLog.LogMessage(si,"Hit3")
							...
							targetRowDetails("Update_User") = si.UserName
								...
								targetRow("Title") = xftvRow.Item("Title").Value
								targetRow("APPN") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateAPPN","")
								targetRow("MDEP") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateMDEP","")
								targetRow("APE9") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateAPEPT","")
								targetRow("Dollar_Type") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateDollarType","")
								targetRow("Obj_Class") = args.CustSubstVarsAlreadyResolved.XFGetValue("ML_CMD_PGM_FormulateObjectClass","")
...
#End Region
#Region "Save MM PEG"
Public Function Save_MM_PEG() As Object
            Dim xftv As New TableView
			xftv = args.TableView
            If xftv Is Nothing OrElse xftv.Rows.Count = 0 Then 
				Return Nothing
...
'								
        ...
        End Function	
#End Region
#Region "Save General Comment"
        Private Function Save_Gen_Cmt() As Object
			Dim REQ_ID_List As New List(Of String) 
			REQ_ID_List = StringHelper.SplitString(args.CustSubstVarsAlreadyResolved.XFGetValue("BL_CMD_PGM_REQTitleList","NA")," ")

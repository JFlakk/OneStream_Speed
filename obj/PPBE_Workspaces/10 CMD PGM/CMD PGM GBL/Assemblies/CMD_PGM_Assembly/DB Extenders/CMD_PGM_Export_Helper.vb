...
Dim sFileHeader As String = $"SCENARIO,NAME,REMARKS,JUSTIFICATION,ISSUE,MDEP,APPN,APE,ROC,DOLLAR_TYPE,BO,RC,CTYPE,UIC,REIMS,REIMC,FSC,FY{yearCols(0)},FY{yearCols(1)},FY{yearCols(2)},FY{yearCols(3)},FY{yearCols(4)},FY{yearCols(5)},FY{yearCols(6)},FY{yearCols(7)},FY{yearCols(8)}"
    ...
    For Each row As DataRow In dtAll.Rows
        ...
        For i As Integer = 0 To 8
            ...
            Dim sSrcMbrScript As String = $"Cb#ARMY:S#{cProbeScenario}:T#{yearCols(i)}:C#Local:E#{sROC}:V#YTD:A#BOR:F#Baseline:I#Top:O#BeforeAdj:U1#{U1_Appn}:U2#{U2_Mdep}:U3#{U3_Ape}:U4#{U4_DollarType}:U5#Top:U6#None:U7#Top:U8#Top"
...
Dim sql As String = $"SELECT '{exportScenario}' AS Scenario,('R') AS BO,Req.Title,Req.Gen_Comments_Notes, Req.Justification,
...
'					 If Not String.IsNullOrWhiteSpace(sMDEP) Then
'						Dim oU2List As List(Of MemberInfo) = BRApi.Finance.Members.GetMembersUsingFilter(si, BRApi.Finance.Dim.GetDimPk(si, "U2_MDEP"), $"U2#PEG.Base.Keep({sMDEP})", True,,)
'						 filterClause = $"AND Req.MDEP IN ({oU2List})"
...
'Fill the DataTable 
Dim sql As String =$"Select '{exportScenario}' AS Scenario, Req.REQ_Type, Req.Title, Req.Description,('R') as BO, Req.MDEP,Req.APPN,Req.APE9,({sROC}) as ROC,
...
Req.Zero_Trust_Cap, Req.Assoc_Directives, Req.Cloud_IND, Req.Strat_Cyber_Sec_PGM,Req.Notes,Dtl.Unit_of_Measure,
...
'BRapi.ErrorLog.LogMessage(si,$"Debug A")		
    ...
    Req.Zero_Trust_Cap, Req.Assoc_Directives, Req.Cloud_IND, Req.Strat_Cyber_Sec_PGM, Req.Notes,
	 MAX(Dtl.Unit_of_Measure) AS UNIT_OF_MEASURE,
	MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower' THEN Dtl.FY_1 ELSE NULL END) AS Manpower_FY_1,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower_Cost' THEN Dtl.FY_1 ELSE NULL END) AS Manpower_FY_1_Cost,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower' THEN Dtl.FY_2 ELSE NULL END) AS Manpower_FY_2,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower_Cost' THEN Dtl.FY_2 ELSE NULL END) AS Manpower_FY_2_Cost,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower' THEN Dtl.FY_3 ELSE NULL END) AS Manpower_FY_3,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower_Cost' THEN Dtl.FY_3 ELSE NULL END) AS Manpower_FY_3_Cost,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower' THEN Dtl.FY_4 ELSE NULL END) AS Manpower_FY_4,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower_Cost' THEN Dtl.FY_4 ELSE NULL END) AS Manpower_FY_4_Cost,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower' THEN Dtl.FY_5 ELSE NULL END) AS Manpower_FY_5,
    MAX(Case When Dtl.Unit_of_Measure = 'Manpower' AND Dtl.Account = 'Manpower_Cost' THEN Dtl.FY_5 ELSE NULL END) AS Manpower_FY_5_Cost

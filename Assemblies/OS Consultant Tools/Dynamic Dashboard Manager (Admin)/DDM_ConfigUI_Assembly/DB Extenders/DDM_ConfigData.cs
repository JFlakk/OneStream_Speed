...
namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.DashboardExtender.DDM_ConfigData
{
	public class MainClass
	{
			...
			try
			{
				...
				switch (args.FunctionType)
				{
					...
					case DashboardExtenderFunctionType.ComponentSelectionChanged:
						...
						if (args.FunctionName.XFEqualsIgnoreCase("ProfileConfig_SaveAdd"))
						{
							return this.ProfileConfig_SaveAdd();
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("Select_Add_DDM_ConfigMenuLayout"))
						{
							return this.Select_Add_DDM_ConfigMenuLayout();
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("Select_DDM_ConfigMenuLayout"))
						{
							return this.Select_DDM_ConfigMenuLayout();
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("ConfigMenuLayout_SaveAdd"))
						{
							return this.ConfigMenuLayout_Save("Add");
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("Select_DDM_Layout_Option_Type"))
						{
							return this.Select_DDM_Layout_Option_Type();
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("Select_Add_DDM_ConfigHdrCtrls"))
						{
							return this.Select_Add_DDM_ConfigHdrCtrls();
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("Select_DDM_Config_Hdr_Ctrl"))
						{
							return this.Select_DDM_Config_Hdr_Ctrl();
						}
						else if (args.FunctionName.XFEqualsIgnoreCase("ConfigHdrCtrls_SaveAdd"))
						{

							return this.ConfigHdrCtrl_SaveAdd();
						}
						break;
				}
				return null;
		...
		private XFSelectionChangedTaskResult ProfileConfig_SaveAdd()
		{
			try
			{
				var save_Data_Task_Result = new XFSelectionChangedTaskResult();

				var wf_ProfileKey = Guid.Parse(args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("IV_DDM_trv_WFP", Guid.Empty.ToString()));
				...
				if (new_Profile_Config_ID == 0)
				{
					...
					using (var connection = new SqlConnection(dbConnApp.ConnectionString))
					{
						...
						if (Convert.ToInt32(DDM_Config_Count_DT.Rows[0]["Count"]) == 0)
						{
							...
							new_Profile_Config_ID = sql_gbl_get_max_id.Get_Max_ID(si, "DDM_Config", "DDM_ConfigID");
							...
							{
							new SqlParameter("@DDM_ConfigID", SqlDbType.Int) { Value = new_Profile_Config_ID }
							};
							...
							newRow["DDM_ConfigID"] = new_Profile_Config_ID;
							newRow["DDM_Type"] = 1;
							...
							{
								newRow["ProfileStepType"] = wfProfile_Step_Type_DT.Rows[0]["ProfileType"];
							}
							newRow["Status"] = 1;
							...
							cmdBuilder.UpdateTable(si, "DDM_Config", DDM_Config_DT, sqa);
						}
					}
				}
		...
		private XFSelectionChangedTaskResult Select_Add_DDM_ConfigMenuLayout()
		{
			try
			{
				...
				DDM_ConfigHelpers.Layout_OptionType optionType = (DDM_ConfigHelpers.Layout_OptionType)optionintValue;

				// 3. Lookup in your LayoutRegistry class
				if (DDM_ConfigHelpers.LayoutRegistry.Configs.TryGetValue(optionType, out var config))
				{
		...
		private XFSelectionChangedTaskResult Select_DDM_ConfigMenuLayout()
		{
			try
			{
				var select_Result = new XFSelectionChangedTaskResult();
				...
				var DDM_ConfigMenuLayout_DT = new DataTable();
				UpdateCustomSubstVar(ref select_Result, "IV_DDM_ConfigMenuUI", "DDM_Config_AddUpdate");
				var dbConnApp = BRApi.Database.CreateApplicationDbConnInfo(si);
				using (var connection = new SqlConnection(dbConnApp.ConnectionString))
				{
					var sqa = new SqlDataAdapter();
					...
					var sqlparams = new SqlParameter[]
					{
						new SqlParameter("@DDM_MenuID", SqlDbType.Int) { Value = existingMenuID }
					};
					sql_gbl_get_datasets.Fill_Get_GBL_DT(si, sqa, DDM_ConfigMenuLayout_DT, sql, sqlparams);
				}
				if (DDM_ConfigMenuLayout_DT.Rows.Count > 0)
				{
					DataRow row = DDM_ConfigMenuLayout_DT.Rows[0];

					// 2. Extract Option_Type and Convert to Enum
					DDM_ConfigHelpers.Layout_OptionType optionType = (DDM_ConfigHelpers.Layout_OptionType)row["Option_Type"];
		...
		private XFSelectionChangedTaskResult Select_Add_DDM_ConfigHdrCtrls()
		{
			try
			{
				var select_Result = new XFSelectionChangedTaskResult();
				select_Result.ChangeCustomSubstVarsInDashboard = true;
				UpdateCustomSubstVar(ref select_Result, "BL_DDM_ConfigHdrCtrls", string.Empty);
				return select_Result;
			}
			catch (Exception ex)
			{
		...
		private XFSelectionChangedTaskResult Select_DDM_Config_Hdr_Ctrl()
		{
			...
			{
				var select_Result = new XFSelectionChangedTaskResult();
				var DDM_ConfigHdrCtrls_DT = new DataTable();
				var configHdrCtrlID = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("BL_DDM_ConfigHdrCtrls", "0").XFConvertToInt();
				select_Result.ChangeCustomSubstVarsInDashboard = true;
				UpdateCustomSubstVar(ref select_Result, "IV_DDM_Hdr_Ctrl_UI", "DDM_ConfigHdr_AddUpdate");
				var dbConnApp = BRApi.Database.CreateApplicationDbConnInfo(si);
				using (var connection = new SqlConnection(dbConnApp.ConnectionString))
				{
					var sqa = new SqlDataAdapter();
					...
					var sql = @"SELECT *
							...
							WHERE DDM_HdrCtrlID = @DDM_HdrCtrlID";
					var sqlparams = new SqlParameter[]
					{
						new SqlParameter("@DDM_HdrCtrlID", SqlDbType.Int) { Value = configHdrCtrlID }
					};
					sql_gbl_get_datasets.Fill_Get_GBL_DT(si, sqa, DDM_ConfigHdrCtrls_DT, sql, sqlparams);
					BRApi.ErrorLog.LogMessage(si, $"Hit here 466 {sql}");
				}

				if (DDM_ConfigHdrCtrls_DT.Rows.Count > 0)
				{
					DataRow row = DDM_ConfigHdrCtrls_DT.Rows[0];
					BRApi.ErrorLog.LogMessage(si, $"Hit here 469");
					// 2. Extract Option_Type and Convert to Enum
					DDM_ConfigHelpers.HdrCtrlType hdrctrlType = (DDM_ConfigHelpers.HdrCtrlType)row["Option_Type"];

					// 3. Lookup in your LayoutRegistry class
					if (DDM_ConfigHelpers.HdrCtrlRegistry.Configs.TryGetValue(hdrctrlType, out var config))
					{
		...
		private XFSelectionChangedTaskResult ConfigMenuLayout_Save(string runType)
		{
			try
			{
				...
				BRApi.ErrorLog.LogMessage(si, $"Hit ConfigID: {configID}, MenuName: {menuName}, SortOrder: {sortOrder}, optionType:{optionType} ");
				...
				using (SqlConnection connection = new SqlConnection(dbConnApp.ConnectionString))
				{
					...
					var sqa = new SqlDataAdapter();
					var DDM_ConfigMenuLayout_DT = new DataTable();
					var sqlSelect = @"SELECT * FROM DDM_ConfigMenuLayout WHERE DDM_MenuID = @DDM_MenuID";
					var sqlparams = new SqlParameter[]
					{
					};
					var cmdBuilder = new GBL_UI_Assembly.SQA_GBL_Command_Builder(si, connection);
					...
					if (runType == "Add")
					{
						var sql_gbl_get_datasets = new GBL_UI_Assembly.SQL_GBL_Get_DataSets(si, connection);
						var sql_gbl_get_max_id = new GBL_UI_Assembly.SQL_GBL_Get_Max_ID(si, connection);
						var newID = sql_gbl_get_max_id.Get_Max_ID(si, "DDM_ConfigMenuLayout", "DDM_MenuID");
						var DDM_Config_DT = new DataTable();
						sqlparams = new SqlParameter[]
						{
						...
						cmdBuilder.FillDataTable(si, sqa, DDM_ConfigMenuLayout_DT, sqlSelect, sqlparams);
						...
						row = DDM_ConfigMenuLayout_DT.NewRow();
						...
						row["DDM_MenuID"] = newID;
						row["Status"] = 1;
						row["CreateDate"] = DateTime.Now;
						row["CreateUser"] = si.UserName;
						row["UpdateDate"] = DateTime.Now;
						row["UpdateUser"] = si.UserName;
						DDM_ConfigMenuLayout_DT.Rows.Add(row);
					}
					else
					{
						cmdBuilder.FillDataTable(si, sqa, DDM_ConfigMenuLayout_DT, sqlSelect, new SqlParameter[] { new SqlParameter("@Menu_ID", existingMenuID) });
						if (DDM_ConfigMenuLayout_DT.Rows.Count == 0) throw new Exception("Record not found.");
						row = DDM_ConfigMenuLayout_DT.Rows[0];
					}

					//	BRApi.ErrorLog.LogMessage(si,$"Hit: {configID} - {optionType} - {menuName} - {sortOrder}");
					// Map standard fields
					row["DDM_ConfigID"] = configID;
					row["Name"] = menuName;
					row["SortOrder"] = sortOrder;
					row["LayoutType"] = optionType;
					row["UpdateDate"] = DateTime.Now;
					row["UpdateUser"] = si.UserName;
					BRApi.ErrorLog.LogMessage(si, $"Hit584");

					DDM_ConfigHelpers.Layout_OptionType layoutType = (DDM_ConfigHelpers.Layout_OptionType)optionType;
					...
					//        Type colType = DDM_ConfigMenuLayout_DT.Columns[targetCol].DataType;
					...
					var valResult = Val_DDM_ConfigMenuLayout(ref row);
					if (!valResult.IsOK) return valResult;
					BRApi.ErrorLog.LogMessage(si, $"Hit: ");
					// 3. Save to DB
					LogDataTable(DDM_ConfigMenuLayout_DT, "Menu");
					cmdBuilder.UpdateTable(si, "DDM_ConfigMenuLayout", DDM_ConfigMenuLayout_DT, sqa);
		...
		private XFSelectionChangedTaskResult ConfigHdrCtrl_SaveAdd()
		{
			...
			try
			{
				...
				var runType = "New";
				var existingHdrCtrlID = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("BL_DDM_ConfigHdrCtrls", "0").XFConvertToInt();
				...
				var optionType = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("DL_DDM_Hdr_Type", "0").XFConvertToInt();
				var menuID = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("BL_DDM_Config_Menu", "0").XFConvertToInt();
				BRApi.ErrorLog.LogMessage(si, $"HERE Save HDR Name {optionType}");
				// 1. Run Duplicate Check before proceeding
				// We 'Initiate' to fill GBL_Menu dictionaries from the DB
				...
				using (SqlConnection connection = new SqlConnection(dbConnApp.ConnectionString))
				{
					...
					var sqa = new SqlDataAdapter();
					var DDM_ConfigHdrCtrls_DT = new DataTable();
					var sqlSelect = "SELECT * FROM DDM_ConfigHdrCtrls WHERE DDM_HdrCtrlID = @DDM_HdrCtrlID";
					var cmdBuilder = new GBL_UI_Assembly.SQA_GBL_Command_Builder(si, connection);
					DataRow row;
					if (runType == "New")
					{
						...
						var sql_gbl_get_datasets = new GBL_UI_Assembly.SQL_GBL_Get_DataSets(si, connection);
						var sql_gbl_get_max_id = new GBL_UI_Assembly.SQL_GBL_Get_Max_ID(si, connection);
						var newID = sql_gbl_get_max_id.Get_Max_ID(si, "DDM_ConfigHdrCtrls", "DDM_HdrCtrlID");
						var DDM_Config_DT = new DataTable();

						cmdBuilder.FillDataTable(si, sqa, DDM_ConfigHdrCtrls_DT, sqlSelect, new SqlParameter[] { new SqlParameter("@DDM_HdrCtrlID", -1) });
						sqlSelect = "SELECT * FROM DDM_Config WHERE DDM_ConfigID = @DDM_ConfigID";
						sql_gbl_get_datasets.Fill_Get_GBL_DT(si, sqa, DDM_Config_DT, sqlSelect, new SqlParameter[] { new SqlParameter("@DDM_ConfigID", SqlDbType.Int) { Value = configID } });
						row = DDM_ConfigHdrCtrls_DT.NewRow();
						DDM_ConfigHdrCtrls_DT.Rows.Add(row);
						if (DDM_Config_DT.Rows.Count > 0)
						{
...

					{
						...
						cmdBuilder.FillDataTable(si, sqa, DDM_ConfigHdrCtrls_DT, sqlSelect, new SqlParameter[] { new SqlParameter("@DDM_HdrCtrlID", existingHdrCtrlID) });
						if (DDM_ConfigHdrCtrls_DT.Rows.Count == 0) throw new Exception("Record not found.");
						row = DDM_ConfigHdrCtrls_DT.Rows[0];
					}
					...
					DDM_ConfigHelpers.HdrCtrlType hdrctrlType = (DDM_ConfigHelpers.HdrCtrlType)row["Option_Type"];
					if (DDM_ConfigHelpers.HdrCtrlRegistry.Configs.TryGetValue(hdrctrlType, out var HdrCtrlConfig))
					{
					...
					var valResult = Val_DDM_ConfigHdrCtrls(ref row);
					if (!valResult.IsOK) return valResult;



					//	LogDataTable(DDM_ConfigHdrCtrls_DT,"Header");
					// cmdBuilder.UpdateTable(si, "DDM_ConfigHdrCtrls", DDM_ConfigHdrCtrls_DT, sqa);

					try
					{
						BRApi.ErrorLog.LogMessage(si, $"FINAL CHECK: Row ID is {row["DDM_HdrCtrlID"]}, Row Status is {row["Status"]}");

						cmdBuilder.UpdateTable(si, "DDM_ConfigHdrCtrls", DDM_ConfigHdrCtrls_DT, sqa);
					}
					catch (Exception ex)
					{
						throw new Exception($"UpdateTable Failed. Table: DDM_ConfigHdrCtrls. Error: {ex.Message} | Inner: {ex.InnerException?.Message}");
					}
		...
		//		public XFSelectionChangedTaskResult Val_DDM_ConfigMenuLayout(ref DataRow row)
		//{
		...
		//        if (!Enum.TryParse(rawLayoutValue, out DDM_ConfigHelpers.Layout_OptionType optionType))
		//        {
		//            throw new Exception($"Value '{rawLayoutValue}' is not a valid Layout_OptionType.");
		//        }
		...
		public XFSelectionChangedTaskResult Val_DDM_ConfigMenuLayout(ref DataRow row)
		{
			...
			try
			{
				...
				// DDM_ConfigHelpers.Layout_OptionType optionType = (DDM_ConfigHelpers.Layout_OptionType)row["LayoutType"];
				string layoutValue = row["LayoutType"] != DBNull.Value ? row["LayoutType"].ToString() : "0";
				DDM_ConfigHelpers.Layout_OptionType optionType;

				if (!Enum.TryParse(layoutValue, out optionType))
				{
					valResult.IsOK = false;
		...
		public XFSelectionChangedTaskResult Val_DDM_ConfigHdrCtrls(ref DataRow row)
		{
			...
			{
				// Parse the Option Type from the row
				DDM_ConfigHelpers.HdrCtrlType optionType = (DDM_ConfigHelpers.HdrCtrlType)row["Option_Type"];

				if (DDM_ConfigHelpers.HdrCtrlRegistry.Configs.TryGetValue(optionType, out var config))
				{
					// 1. Identify "Whitelist" columns (Target DB columns)
					var requiredColumns = config.ParameterMappings.Values
						.SelectMany(d => d.Values)

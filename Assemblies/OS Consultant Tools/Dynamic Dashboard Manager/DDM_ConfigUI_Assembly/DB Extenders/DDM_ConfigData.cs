...
namespace Workspace.__WsNamespacePrefix.__WsAssemblyName.BusinessRule.DashboardExtender.DDM_ConfigData
{
	public class MainClass
	{
			...
			try
			{
				...
				switch (args.FunctionType)
				{
					...
					case DashboardExtenderFunctionType.ComponentSelectionChanged:
						...
						else if (args.FunctionName.XFEqualsIgnoreCase("Save_DDM_ConfigHdrCtrls"))
						{
							return this.Save_DDM_ConfigHdrCtrls();
						}
						break;
				}
				return null;
		...
		private XFSelectionChangedTaskResult Save_DDM_Config_Menu_Layout()
		{
			try
			{
				...
				using (SqlConnection connection = new SqlConnection(dbConnApp.ConnectionString))
				{
					...
					var sqa = new SqlDataAdapter();
					var DDM_Config_Menu_Layout_DT = new DataTable();
					var sqlSelect = "SELECT * FROM DDM_Config_Menu_Layout WHERE DDM_MenuID = @Menu_ID";
					var cmdBuilder = new GBL_UI_Assembly.SQA_GBL_Command_Builder(si, connection);
					DataRow row;
					if (runType == "New")
					{
						var sql_gbl_get_datasets = new GBL_UI_Assembly.SQL_GBL_Get_DataSets(si, connection);
						var sql_gbl_get_max_id = new GBL_UI_Assembly.SQL_GBL_Get_Max_ID(si, connection);
						var newID = sql_gbl_get_max_id.Get_Max_ID(si, "DDM_Config_Menu_Layout", "DDM_MenuID");
						var DDM_Config_DT = new DataTable();
					...
					// 3. Save to DB
					cmdBuilder.UpdateTableSimple(si, "DDM_Config_Menu_Layout", DDM_Config_Menu_Layout_DT, sqa, "DDM_MenuID");
		...
		private XFSelectionChangedTaskResult Save_DDM_ConfigHdrCtrls()
		{
			try
			{
				var runType = "New";

        ...
        public void Update_DDM_Config_Menu_Hdr(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "DDM_Config_Menu_Hdr", dt);
                    builder.SetPrimaryKey("Menu_Hdr_ID");
                    builder.ExcludeFromUpdate("Menu_Hdr_ID", "DDM_Config_ID", "Create_Date", "Create_User");

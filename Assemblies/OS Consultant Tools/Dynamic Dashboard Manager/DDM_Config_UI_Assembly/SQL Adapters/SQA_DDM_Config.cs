        ...
        public void Update_DDM_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "DDM_Config", dt);
                    builder.SetPrimaryKey("DDM_Config_ID");
                    builder.ExcludeFromUpdate("DDM_Config_ID", "Create_Date", "Create_User");

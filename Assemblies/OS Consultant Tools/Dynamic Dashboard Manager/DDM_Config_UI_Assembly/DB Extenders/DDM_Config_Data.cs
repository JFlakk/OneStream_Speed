		...
		private XFSelectionChangedTaskResult Select_DDM_Config_Menu_Layout()
		{
			try
			{
				...
				if (ddm_Config_Menu_Layout_DT.Rows.Count > 0)
				{
					...
					DDM_Config_Helpers.Layout_OptionType optionType = (DDM_Config_Helpers.Layout_OptionType)row["Layout_Type"];
					UpdateCustomSubstVar(ref select_Result, "DL_DDM_Layout_Type", row["Layout_Type"].ToString());
					UpdateCustomSubstVar(ref select_Result, "IV_DDM_Layout_Workspace_ID", row["Workspace_ID"].ToString());
					UpdateCustomSubstVar(ref select_Result, "IV_DDM_Layout_MaintUnit_ID", row["MaintUnit_ID"].ToString());
					UpdateCustomSubstVar(ref select_Result, "IV_DDM_Layout_Dashboard_ID", row["Dashboard_ID"].ToString());
					UpdateCustomSubstVar(ref select_Result, "IV_DDM_Layout_Profile_Key", row["Profile_Key"].ToString());
					// 3. Lookup in your LayoutRegistry class
					if (DDM_Config_Helpers.LayoutRegistry.Configs.TryGetValue(optionType, out var config))
					{
...
private XFSelectionChangedTaskResult Save_DDM_Config_Menu_Layout()
{
	try
	{
		...
		using (SqlConnection connection = new SqlConnection(dbConnApp.ConnectionString))
		{
			...
			row["Layout_Type"] = optionType;
			var uiWorkspaceId = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("IV_DDM_Layout_Workspace_ID", Convert.ToString(row["Workspace_ID"]));
			row["Workspace_ID"] = string.IsNullOrWhiteSpace(uiWorkspaceId) ? (object)DBNull.Value : uiWorkspaceId;
			var uiMaintUnitId = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("IV_DDM_Layout_MaintUnit_ID", Convert.ToString(row["MaintUnit_ID"]));
			row["MaintUnit_ID"] = string.IsNullOrWhiteSpace(uiMaintUnitId) ? (object)DBNull.Value : uiMaintUnitId;
			var uiDashboardId = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("IV_DDM_Layout_Dashboard_ID", Convert.ToString(row["Dashboard_ID"]));
			row["Dashboard_ID"] = string.IsNullOrWhiteSpace(uiDashboardId) ? (object)DBNull.Value : uiDashboardId;
			var uiProfileKey = args.SelectionChangedTaskInfo.CustomSubstVars.XFGetValue("IV_DDM_Layout_Profile_Key", Convert.ToString(row["Profile_Key"]));
			row["Profile_Key"] = string.IsNullOrWhiteSpace(uiProfileKey) ? (object)DBNull.Value : uiProfileKey;
			row["Update_Date"] = DateTime.Now;
			row["Update_User"] = si.UserName;

    ...
    public class Global_Functions : IWsasFinanceCustomCalculateV800
    {
        ...
        private int UpdateDimension(int MemberID, string dimensionValue, Dim dimension, FinanceRulesApi api)
        {
            if (!string.IsNullOrEmpty(dimensionValue) && dimensionValue != "NoPassedValue")
            {
                ...
                {
                    MemberID = GetMember(ref dimension, "XFCommon", ref api).MemberId;
                }
                else
                {
                    MemberID = GetMember(ref dimension, dimensionValue, ref api).MemberId;
                }
            }
            return MemberID;
        }
            ...
            )
        {
            string bal_buffer_cell_SQL = @"
												Select Acct, View,Origin,IC,Flow,UD1,
													...
													OS_Curr_Cube_Buffer_Filter,BufferFilter
													FROM MCM_Cell
													WHERE CalcID = @CalcID";
            ...
            using (var connection = new SqlConnection(dbConnApp.ConnectionString))
            {
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        var Buffer = reader["BufferFilter"].ToString();
            ...
            )
        {
            ...
            string UnbalBuffer_cell_SQL = @"
												Select Acct, View,Origin,IC,Flow,UD1,
													...
													WHERE CalcID = @CalcID";
            string src_cell_SQL = @"
						        SELECT Calc_Src_ID_Order, OpenParens, MathOperator, Entity, Cons, Scenario,
									...
									Unbal_UD7_Override,Unbal_UD8_Override,Unbal_Src_Cell_Buffer_Filter,OS_Dynamic_Calc_Script,OverrideValue,
									Calc_Src_Type
						        FROM MCM_Src_Cell
						        WHERE CalcID = @CalcID
            ...
            using (var connection = new SqlConnection(dbConnApp.ConnectionString))
            {
                ...
                using (var unbal_srcreader = unbal_srcCommand.ExecuteReader())
                {
                    while (unbal_srcreader.Read())
                    {
                        ...
                        row["Time"] = unbal_srcreader["Time"];
                        row["Origin"] = unbal_srcreader["Origin"];
                        row["IC"] = unbal_srcreader["IC"];
                        row["View"] = unbal_srcreader["View"];
                        row["Acct"] = unbal_srcreader["Acct"];
                        row["Flow"] = unbal_srcreader["Flow"];
                        row["UD1"] = unbal_srcreader["UD1"];
                        row["UD2"] = unbal_srcreader["UD2"];
                        row["UD3"] = unbal_srcreader["UD3"];
                        row["UD4"] = unbal_srcreader["UD4"];
                        row["UD5"] = unbal_srcreader["UD5"];
                        ...
                        row["Unbal_Acct_Override"] = unbal_srcreader["Unbal_Acct_Override"];
                        row["Unbal_Flow_Override"] = unbal_srcreader["Unbal_Flow_Override"];
                        row["Unbal_UD1_Override"] = unbal_srcreader["Unbal_UD1_Override"];
                        row["Unbal_UD2_Override"] = unbal_srcreader["Unbal_UD2_Override"];
                        row["Unbal_UD3_Override"] = unbal_srcreader["Unbal_UD3_Override"];
                        row["Unbal_UD4_Override"] = unbal_srcreader["Unbal_UD4_Override"];
                        row["Unbal_UD5_Override"] = unbal_srcreader["Unbal_UD5_Override"];
                        row["Unbal_UD6_Override"] = unbal_srcreader["Unbal_UD6_Override"];
                        ...
                        row["OverrideValue"] = unbal_srcreader["OverrideValue"];
                        ...
                        if (calc_srcType != "Dynamic Calc")
                        {
                            ...
                            string unbal_srcBuffer = unbal_srcreader["Unbal_Src_Cell_Buffer"].ToString();
                            string unbal_srcBufferFilter = unbal_srcreader["BufferFilter"].ToString();
                                        ...
                                        if (mbrList3_Dim != null)
                                        {
                                            ...
                                            foreach (var member3 in mem3List)
                                            {
                                                ...
                                                if (mbrList4_Dim != null)
                                                {
                                                    ...
                                                    {
                                                        ExecuteDataBufferCode(CalcID, ref currCubeBuffer, ref destBuffer, ref clearCubeData, destInfo, member1.Member.Name, member2.Member.Name, member3.Member.Name, member4.Member.Name, reader, bal_buffer_calc, UnbalCalc);
                                                    }

                                                }
                                                else
                                                {
                                                    ExecuteDataBufferCode(CalcID, ref currCubeBuffer, ref destBuffer, ref clearCubeData, destInfo, member1.Member.Name, member2.Member.Name, member3.Member.Name, "", reader, bal_buffer_calc, UnbalCalc);
                                                }
                                            }
                                        }
                                        else
                                        {
                                            ExecuteDataBufferCode(CalcID, ref currCubeBuffer, ref destBuffer, ref clearCubeData, destInfo, member1.Member.Name, member2.Member.Name, "", "", reader, bal_buffer_calc, UnbalCalc);
                                        }
                                    }
                                }
                                else
                                {
                                    ExecuteDataBufferCode(CalcID, ref currCubeBuffer, ref destBuffer, ref clearCubeData, destInfo, member1.Member.Name, "", "", "", reader, bal_buffer_calc, UnbalCalc);
                                }
                            }
                        }
            ...
            SqlDataReader reader, string bal_buffer_calc, string UnbalCalc)
        {
            ...
            using (var connection = new SqlConnection(dbConnApp.ConnectionString))
            {
                ...
                using (var unbal_srcreader = unbal_srcCommand.ExecuteReader())
                {
                    ...
                    if (calc_srcType != "Dynamic Calc")
                    {
                        string unbal_srcBuffer = row["Unbal_Src_Cell_Buffer"].ToString();
                        string unbal_srcBufferFilter = row["Unbal_Src_Cell_Buffer_Filter"].ToString();
                        string bufferName = $"SrcBuffer{index}";
                ...
                string acct = destrow["Acct"] != DBNull.Value ? destrow["Acct"].ToString() : null;
                string view = destrow["View"] != DBNull.Value ? destrow["View"].ToString() : null;
                string origin = destrow["Origin"] != DBNull.Value ? destrow["Origin"].ToString() : null;
                string ic = destrow["IC"] != DBNull.Value ? destrow["IC"].ToString() : null;
                string flow = destrow["Flow"] != DBNull.Value ? destrow["Flow"].ToString() : null;
                string ud1 = destrow["UD1"] != DBNull.Value ? destrow["UD1"].ToString() : null;
                string ud2 = destrow["UD2"] != DBNull.Value ? destrow["UD2"].ToString() : null;
                string ud3 = destrow["UD3"] != DBNull.Value ? destrow["UD3"].ToString() : null;
                string ud4 = destrow["UD4"] != DBNull.Value ? destrow["UD4"].ToString() : null;
                string ud5 = destrow["UD5"] != DBNull.Value ? destrow["UD5"].ToString() : null;
                string ud6 = destrow["UD6"] != DBNull.Value ? destrow["UD6"].ToString() : null;
                string ud7 = destrow["UD7"] != DBNull.Value ? destrow["UD7"].ToString() : null;
                string ud8 = destrow["UD8"] != DBNull.Value ? destrow["UD8"].ToString() : null;

                List<string> destInfos = new List<string> {
                        acct, view, origin, ic, flow, ud1, ud2, ud3, ud4, ud5, ud6, ud7, ud8
                    };
        ...
        private decimal getdynamic_calc_value(DataBufferCell balanced_Src_Cell, DataRow srcRow)
        {
            ...
            {
                dyncalc_mbrScriptBuilder.SetOrigin(srcRow["Origin"].ToString().Replace("O#", string.Empty));
            }

            if (srcRow["Unbal_IC_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_IC_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetIC(srcRow["IC"].ToString().Replace("IC#", string.Empty));
            }

            if (srcRow["Unbal_Acct_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_Acct_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetAccount(srcRow["Acct"].ToString().Replace("A#", string.Empty));
            }

            if (srcRow["Unbal_Flow_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_Flow_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetFlow(srcRow["Flow"].ToString().Replace("F#", string.Empty));
            }

            if (srcRow["Unbal_UD1_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD1_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD1(srcRow["UD1"].ToString().Replace("UD1#", string.Empty).Replace("U1#", string.Empty));
            }

            if (srcRow["Unbal_UD2_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD2_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD2(srcRow["UD2"].ToString().Replace("UD2#", string.Empty).Replace("U2#", string.Empty));
            }

            if (srcRow["Unbal_UD3_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD3_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD3(srcRow["UD3"].ToString().Replace("UD3#", string.Empty).Replace("U3#", string.Empty));
            }

            if (srcRow["Unbal_UD4_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD4_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD4(srcRow["UD4"].ToString().Replace("UD4#", string.Empty).Replace("U4#", string.Empty));
            }

            if (srcRow["Unbal_UD5_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD5_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD5(srcRow["UD5"].ToString().Replace("UD5#", string.Empty).Replace("U5#", string.Empty));
            }

            if (srcRow["Unbal_UD6_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD6_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD6(srcRow["UD6"].ToString().Replace("UD6#", string.Empty).Replace("U6#", string.Empty));
            }

            if (srcRow["Unbal_UD7_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD7_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD7(srcRow["UD7"].ToString().Replace("UD7#", string.Empty).Replace("U7#", string.Empty));
            }

            if (srcRow["Unbal_UD8_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD8_Override"].ToString()))
            {
                dyncalc_mbrScriptBuilder.SetUD8(srcRow["UD8"].ToString().Replace("UD8#", string.Empty).Replace("U8#", string.Empty));
            }
        ...
        private decimal get_src_UnbalBuffer_value(DataBufferCell balanced_Src_Cell, DataBuffer Unbal_DataBuffer, DataRow srcRow)
        {
            ...
            if (srcRow["Unbal_Acct_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_Acct_Override"].ToString()))
            {
                DriverDB_Acct = srcRow["Acct"].ToString().Replace("A#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_Flow = string.Empty;
            if (srcRow["Unbal_Flow_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_Flow_Override"].ToString()))
            {
                DriverDB_Flow = srcRow["Flow"].ToString().Replace("F#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_Origin = string.Empty;
            if (srcRow["Unbal_Origin_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_Origin_Override"].ToString()))
            {
                DriverDB_Origin = srcRow["Origin"].ToString().Replace("O#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_IC = string.Empty;
            if (srcRow["Unbal_IC_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_IC_Override"].ToString()))
            {
                DriverDB_IC = srcRow["IC"].ToString().Replace("IC#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD1 = string.Empty;
            if (srcRow["Unbal_UD1_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD1_Override"].ToString()))
            {
                DriverDB_UD1 = srcRow["UD1"].ToString().Replace("UD1#", string.Empty).Replace("U1#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD2 = string.Empty;
            if (srcRow["Unbal_UD2_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD2_Override"].ToString()))
            {
                DriverDB_UD2 = srcRow["UD2"].ToString().Replace("UD2#", string.Empty).Replace("U2#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD3 = string.Empty;
            if (srcRow["Unbal_UD3_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD3_Override"].ToString()))
            {
                DriverDB_UD3 = srcRow["UD3"].ToString().Replace("UD3#", string.Empty).Replace("U3#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD4 = string.Empty;
            if (srcRow["Unbal_UD4_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD4_Override"].ToString()))
            {
                DriverDB_UD4 = srcRow["UD4"].ToString().Replace("UD4#", string.Empty).Replace("U4#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD5 = string.Empty;
            if (srcRow["Unbal_UD5_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD5_Override"].ToString()))
            {
                DriverDB_UD5 = srcRow["UD5"].ToString().Replace("UD5#", string.Empty).Replace("U5#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD6 = string.Empty;
            if (srcRow["Unbal_UD6_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD6_Override"].ToString()))
            {
                DriverDB_UD6 = srcRow["UD6"].ToString().Replace("UD6#", string.Empty).Replace("U6#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD7 = string.Empty;
            if (srcRow["Unbal_UD7_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD7_Override"].ToString()))
            {
                DriverDB_UD7 = srcRow["UD7"].ToString().Replace("UD7#", string.Empty).Replace("U7#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
            var DriverDB_UD8 = string.Empty;
            if (srcRow["Unbal_UD8_Override"] != DBNull.Value && !string.IsNullOrEmpty(srcRow["Unbal_UD8_Override"].ToString()))
            {
                DriverDB_UD8 = srcRow["UD8"].ToString().Replace("UD8#", string.Empty).Replace("U8#", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty);
            }
        ...
        private void UpdateExpressionForRow(DataBufferCell balBuffer_Cell, DataRow srcRow, Dictionary<string, DataBuffer> unbal_srcdataBuffers, ref Dictionary<DataBufferCell, string> cell_Expressions, DataBufferCell Cell, int i)
        {
            ...
            else if (srcRow["Calc_Src_Type"].ToString() == "Stored Cell")
            {
                ...
                var unbal_alloc_filter_conditions = get_filter_Conditions(balBuffer_Cell, srcRow);
                ...
                {
                    if (srcRow["OverrideValue"] != DBNull.Value)
                    {
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("A#"))
                        {
                            acct = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetAccountName(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("O#"))
                        {
                            origin = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetOriginName(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("F#"))
                        {
                            flow = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetFlowName(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("IC#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("I#"))
                        {
                            ic = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetICName(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U1#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD1#"))
                        {
                            ud1 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD1Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U2#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD2#"))
                        {
                            ud2 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD2Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U3#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD3#"))
                        {
                            ud3 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD3Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U4#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD4#"))
                        {
                            ud4 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD4Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U5#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD5#"))
                        {
                            ud5 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD5Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U6#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD6#"))
                        {
                            ud6 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD6Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U7#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD7#"))
                        {
                            ud7 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD7Name(api);
                        }
                        if (srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("U8#") || srcRow["OverrideValue"].ToString().XFContainsIgnoreCase("UD8#"))
                        {
                            ud8 = unbal_Alloc_Buffer_Cell.DataBufferCellPk.GetUD8Name(api);
                        }
                        var unbal_alloc_DestCell = new DataBufferCell(UpdateCellDefinition(Cell, acct, flow, origin,
                                ic, ud1, ud2, ud3, ud4, ud5, ud6, ud7, ud8));
                        curr_expression = cell_Expressions[Cell];
                        curr_expression = curr_expression.Replace($"SrcBuffer{i}", unbal_Alloc_Buffer_Cell.CellAmount.XFToString());
                        cell_Expressions.Add(unbal_alloc_DestCell, curr_expression);

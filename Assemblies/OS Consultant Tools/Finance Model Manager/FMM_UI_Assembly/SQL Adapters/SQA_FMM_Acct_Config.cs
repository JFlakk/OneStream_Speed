    ...
    public class SQA_FMM_Acct_Config
    {
        ...
        public void Update_FMM_Acct_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                ...
                string deleteQuery = @"
                    DELETE FROM FMM_Acct_Config 
                    WHERE Acct_ID = @Acct_ID";
                sqa.DeleteCommand = new SqlCommand(deleteQuery, _connection, transaction);
                sqa.DeleteCommand.Parameters.Add(new SqlParameter("@Acct_ID", SqlDbType.Int) { SourceColumn = "Acct_ID", SourceVersion = DataRowVersion.Original });

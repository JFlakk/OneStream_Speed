        ...
        public void Update_FMM_Acct_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Acct_Config", dt);
                    builder.SetPrimaryKey("Acct_ID");
                    builder.ExcludeFromUpdate("Acct_ID", "Cube_ID", "Act_ID", "Unit_ID", "Create_Date", "Create_User");

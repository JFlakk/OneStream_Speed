        ...
        public void Update_FMM_Src_Cell(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Src_Cell", dt);
                    builder.SetPrimaryKey("Src_Cell_ID");
                    builder.ExcludeFromUpdate("Src_Cell_ID", "Cube_ID", "Reg_ID", "Create_Date", "Create_User");

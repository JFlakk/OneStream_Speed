        ...
        public void Update_FMM_Calc_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Calc_Config", dt);
                    builder.SetPrimaryKey("Calc_ID");
                    builder.ExcludeFromUpdate("Calc_ID", "Cube_ID", "Act_ID", "Create_Date", "Create_User");

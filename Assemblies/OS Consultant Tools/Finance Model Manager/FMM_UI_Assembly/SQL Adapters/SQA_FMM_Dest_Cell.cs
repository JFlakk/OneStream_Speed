        ...
        public void Update_FMM_Dest_Cell(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Dest_Cell", dt);
                    builder.SetPrimaryKey("Dest_Cell_ID");
                    builder.ExcludeFromUpdate("Dest_Cell_ID", "Cube_ID", "Reg_ID", "Create_Date", "Create_User");

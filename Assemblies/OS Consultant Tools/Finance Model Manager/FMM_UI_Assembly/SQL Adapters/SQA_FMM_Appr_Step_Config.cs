        ...
        public void Update_FMM_Appr_Step_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Appr_Step_Config", dt);
                    builder.SetPrimaryKey("Appr_Step_ID");
                    builder.ExcludeFromUpdate("Appr_Step_ID", "Cube_ID", "Appr_ID", "Create_Date", "Create_User");

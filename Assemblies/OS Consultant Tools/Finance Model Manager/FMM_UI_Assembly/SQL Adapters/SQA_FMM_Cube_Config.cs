        ...
        public void Update_FMM_Cube_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Cube_Config", dt);
                    builder.SetPrimaryKey("Cube_ID");
                    builder.ExcludeFromUpdate("Cube_ID", "Create_Date", "Create_User");

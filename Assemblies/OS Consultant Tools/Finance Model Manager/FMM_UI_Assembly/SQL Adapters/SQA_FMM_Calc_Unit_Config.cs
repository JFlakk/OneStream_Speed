        ...
        public void Update_FMM_Calc_Unit_Config(SessionInfo si, DataTable dt, SqlDataAdapter sqa)
        {
            using (SqlTransaction transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    var builder = new GBL_SQL_Command_Builder(_connection, "FMM_Calc_Unit_Config", dt);
                    builder.SetPrimaryKey("Calc_Unit_ID");
                    builder.ExcludeFromUpdate("Calc_Unit_ID", "Cube_ID", "Create_Date", "Create_User");
